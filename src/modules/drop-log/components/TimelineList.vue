<template>
  <RecycleScroller
    ...
    v-slot="{ item }"
  >
    <article class="surface-card mx-3 my-2 p-3 transition-all duration-200 hover:-translate-y-0.5">
      <header class="mb-1 flex items-center justify-between text-xs text-slate-400">
        ...
        <button class="text-rose-300 transition hover:text-rose-200" @click="$emit('delete', item.id)">
          Delete
        </button>
      </header>
