  ...
  <section class="relative flex min-h-screen flex-col overflow-hidden px-5 pb-8 pt-10 text-slate-900 dark:text-slate-100">
    ...
    <div class="relative z-10 mx-auto flex w-full max-w-md flex-1 flex-col">
      <header class="glass-panel mb-5 flex items-center gap-3 rounded-2xl px-4 py-3">
        <img src="/logo.svg" alt="DropLog logo" class="h-11 w-11 rounded-xl shadow-md ring-1 ring-white/70" />
        <div>
      ...
      <nav class="glass-nav mb-5 grid grid-cols-3 gap-1 rounded-2xl p-1.5">
        <button
          ...
          :class="currentPage === index ? 'bg-white/90 text-slate-900 dark:bg-slate-200 dark:text-slate-950' : 'text-slate-700 dark:text-slate-300'"
      ...
      <article class="glass-panel flex-1 rounded-3xl p-5">
        <p class="mb-2 text-xs uppercase tracking-[0.25em] text-slate-600 dark:text-slate-400">Core use {{ currentPage + 1 }}/3</p>
        <h2 class="text-2xl font-bold">{{ activePage.title }}</h2>
        <p class="mt-3 text-sm leading-relaxed text-slate-700 dark:text-slate-300">{{ activePage.description }}</p>
